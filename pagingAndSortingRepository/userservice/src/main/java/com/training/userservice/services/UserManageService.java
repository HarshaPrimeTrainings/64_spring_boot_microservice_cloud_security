...
public class UserManageService {
	...
	public List<User> findAllusersPage(int pageNumber,int pageSize){
		Pageable peagable = PageRequest.of(pageNumber, pageSize);
		Page<User>  page= userRepo.findAll(peagable);
		return page.toList();
	}
	
	public List<User> findSortedusers(String column,String sortOrder){
		if(sortOrder.equals("desc")) {
			return  userRepo.findAll(Sort.by(column).descending());
		}
		return  userRepo.findAll(Sort.by(column));
	}
